...
export function computeDashboardSummary(
  ...
  const effectiveBucketMode: "daily" | "monthly" | "yearly" =
    monthsSpan >= 24 ? "yearly" : bucketMode;
  ...
  const makeBucketizer = () => {
    ...
    const pushMonths = (start: Date, end: Date) => {
    ...
    if (windowKind === "week") {
      bFrom = new Date(from.setHours(0, 0, 0, 0));
      bTo = new Date(to.setHours(23, 59, 59, 999));
      const d = new Date(bFrom);
      while (d <= bTo) {
        labels.push(format(d, "dd/MM/yyyy"));
        d.setDate(d.getDate() + 1);
      }
    } else if (windowKind === "month") {
      bFrom = startOfMonth(from);
      bTo = endOfMonth(from);
      labels = ["Week 1", "Week 2", "Week 3", "Week 4"];
    } else if (windowKind === "year") {
      bFrom = startOfYear(from);
      bTo = endOfYear(from);
      pushMonths(bFrom, bTo);
    } else if (effectiveBucketMode === "yearly") {
      bFrom = startOfYear(from);
      bTo = endOfYear(to);
      let y = startOfYear(bFrom);
      while (y <= bTo) {
        labels.push(format(y, "yyyy"));
        y = addYears(y, 1);
      }
    } else if (effectiveBucketMode === "monthly") {
      bFrom = startOfMonth(addMonths(to, -11));
      bTo = endOfMonth(to);
      pushMonths(bFrom, bTo);
    } else {
    ...
    const keyFor = (ds: string) => {
      const d = parseCsvDate(ds)!;
      if (windowKind === "week") return format(d, "dd/MM/yyyy");
      if (windowKind === "month") {
        const day = d.getDate();
      ...
      if (windowKind === "year" || effectiveBucketMode === "monthly")
        return format(d, "MMM yyyy");
      if (effectiveBucketMode === "daily") return format(d, "dd/MM/yyyy");
  ...
  const dayTotals = new Map<
    string,
    { date: Date; views: number; completions: number }
  >();
  ...
  for (const row of events) {
    ...
    if (inSel) {
      ...
      const kid = row?.knowby_id as string | undefined;
      const kname = row?.knowby_name as string | undefined;
      if (kid) {
        const prev = knowbyAgg.get(kid) ?? {
          name: kname,
          views: 0,
          completions: 0,
        };
        if (kind === "views") prev.views += 1;
        else prev.completions += 1;
        if (!prev.name && kname) prev.name = kname;
        knowbyAgg.set(kid, prev);
        knowbySet.add(kid);
      }
      const mid = row?.member_id as string | undefined;
      const mname = row?.member_name as string | undefined;
      if (mid) {
        const prevM = memberAgg.get(mid) ?? { name: mname, actions: 0 };
        prevM.actions += 1;
        if (!prevM.name && mname) prevM.name = mname;
        memberAgg.set(mid, prevM);
        memberSet.add(mid);
      }
      ...
      if (dt) {
        const ymd = `${dt.getFullYear()}-${String(dt.getMonth() + 1).padStart(
          2,
          "0"
        )}-${String(dt.getDate()).padStart(2, "0")}`;
        if (!dayTotals.has(ymd))
          dayTotals.set(ymd, { date: new Date(dt), views: 0, completions: 0 });
        const rec = dayTotals.get(ymd)!;
        if (kind === "views") rec.views += 1;
        else rec.completions += 1;
    ...
    if (inBucket) {
      const key = ds ? bucketKey(ds) : undefined;
      if (key && buckets.has(key)) {
        const b = buckets.get(key)!;
        if (kind === "views") b.views += 1;
        else b.completions += 1;
      }
    }
  }
  ...
  for (const row of views) {
    if (inRangeDate(row?.date, prev.from, prev.to)) {
      pv += 1;
  ...
  const deltaMembersBase =
    memberSet.size === 0 && pmemberSet.size === 0
      ? null
      : pmemberSet.size > 0
      ? ((memberSet.size - pmemberSet.size) / pmemberSet.size) * 100
      : 100;
  const deltaKnowbysBase =
    knowbySet.size === 0 && pknowbySet.size === 0
      ? null
      : pknowbySet.size > 0
      ? ((knowbySet.size - pknowbySet.size) / pknowbySet.size) * 100
      : 100;
  const deltaViewsBase =
    v === 0 && pv === 0 ? null : pv > 0 ? ((v - pv) / pv) * 100 : 100;
  const deltaCompletionsBase =
    c === 0 && pc === 0 ? null : pc > 0 ? ((c - pc) / pc) * 100 : 100;
  const deltaRateBase =
    completionRate === 0 && prevRate === 0 ? null : completionRate - prevRate;

  const hasPrevData =
    pv > 0 || pc > 0 || pmemberSet.size > 0 || pknowbySet.size > 0;
  ...
  const bucketArray = bucketLabels.map((l) => ({
    label: l,
    views: buckets.get(l)?.views ?? 0,
    completions: buckets.get(l)?.completions ?? 0,
  }));

  // 5) Work out the simple insights (busiest day/weekday/hour + averages)
  const days = Array.from(dayTotals.values()).sort(
    (a, b) => a.date.getTime() - b.date.getTime()
  );
  let peakDate: { date: Date; total: number } | null = null;
  for (const d of days) {
    const total = d.views + d.completions;
    if (!peakDate || total > peakDate.total) peakDate = { date: d.date, total };
  }
  const sumViews = days.reduce((s, d) => s + d.views, 0);
  const sumComps = days.reduce((s, d) => s + d.completions, 0);
  const avgViews = days.length ? sumViews / days.length : 0;
  const avgComps = days.length ? sumComps / days.length : 0;
  const weekdayNames = [
    "Sunday",
    "Monday",
    "Tuesday",
  ...
  let peakWeekday: { name: string; total: number } | null = null;
  for (let i = 0; i < 7; i++) {
    if (!peakWeekday || weekdayCounts[i] > (peakWeekday?.total ?? -1))
      peakWeekday = { name: weekdayNames[i], total: weekdayCounts[i] };
  }
  let peakHour: { hour: number; total: number } | null = null;
  if (anyTime) {
    for (let h = 0; h < 24; h++) {
      if (!peakHour || hourCounts[h] > (peakHour?.total ?? -1))
        peakHour = { hour: h, total: hourCounts[h] };
    }
  }
  ...
  const isAllTime = !!(
    datasetMin &&
    datasetMax &&
    isSameDay(from, datasetMin) &&
    isSameDay(to, datasetMax)
  );
  const periodLabel = isAllTime
    ? `All time (${format(from, "dd MMM yyyy")} to ${format(
        to,
        "dd MMM yyyy"
      )})`
...
export function exportDashboardPdf(
    ...
    const drawHeader = (yHeaderTop: number) => {
      ...
      for (let i = 0; i < header.length; i++) {
        ...
        if (numericCols.includes(i))
          rightAlign(text, cx + colWidths[i] - cellPadX, headerBaseline);
    ...
    for (let r = 0; r < rows.length; r++) {
      ...
      for (let c = 0; c < row.length; c++) {
        ...
        if (numericCols.includes(c)) {
          ...
          rightAlign(
            lines[0],
            cx2 + colWidths[c] - cellPadX,
            yyTop + cellPadY + lineH - 2
          );
        } else {
  ...
  const renderSimpleTableSection = (
    ...
    const firstCol = Math.max(200, available - fixedColsWidth);
    const colWidths = [
      firstCol,
      ...header
        .slice(1)
        .map((_, i) => (i === header.length - 2 ? fixedColsWidth - 80 : 80)),
    ];
  ...
  const kpiItems: Array<[string, string, string | null]> = [
    [
      "Active Members",
      String(summary.kpis.activeMembers),
      summary.deltas.membersPct == null
        ? null
        : `${
            summary.deltas.membersPct > 0 ? "+" : ""
          }${summary.deltas.membersPct.toFixed(2)}%`,
    ],
    [
      "Knowbys",
      String(summary.kpis.knowbys),
      summary.deltas.knowbysPct == null
        ? null
        : `${
            summary.deltas.knowbysPct > 0 ? "+" : ""
          }${summary.deltas.knowbysPct.toFixed(2)}%`,
    ],
    [
      "Views",
      String(summary.kpis.views),
      summary.deltas.viewsPct == null
        ? null
        : `${
            summary.deltas.viewsPct > 0 ? "+" : ""
          }${summary.deltas.viewsPct.toFixed(2)}%`,
    ],
    [
      "Completions",
      String(summary.kpis.completions),
      summary.deltas.completionsPct == null
        ? null
        : `${
            summary.deltas.completionsPct > 0 ? "+" : ""
          }${summary.deltas.completionsPct.toFixed(2)}%`,
    ],
    [
      "Completion Rate",
      pct(summary.kpis.completionRate || 0),
      summary.deltas.rateDelta == null
        ? null
        : `${
            summary.deltas.rateDelta > 0 ? "+" : ""
          }${summary.deltas.rateDelta.toFixed(2)} pts`,
    ],
  ];
  ...
  if (
    ...
    summary.kpis.views === 0 &&
    summary.kpis.completions === 0
  ) {
    drawEmptyBox(
    ...
    if (insights.peakDate)
      bullets.push(
        `Busiest day: ${format(
          insights.peakDate.date,
          "dd MMM yyyy"
        )} (${insights.peakDate.total.toLocaleString()} interactions)`
      );
    if (insights.peakWeekday)
      bullets.push(
        `Most active weekday: ${
          insights.peakWeekday.name
        } (${insights.peakWeekday.total.toLocaleString()} total)`
      );
    if (insights.peakHour)
      bullets.push(
        `Peak hour: ${String(insights.peakHour.hour).padStart(
          2,
          "0"
        )}:00–${String((insights.peakHour.hour + 1) % 24).padStart(2, "0")}:00`
      );
    else bullets.push("Peak hour: insufficient timestamp data to determine.");
    bullets.push(
      `Average per day: ${insights.avgViews.toFixed(
        1
      )} views, ${insights.avgComps.toFixed(1)} completions`
    );
  ...
  // ===== Top Members =====
  const tm = summary.topMembersByActivity;
  const tmRows = tm.map((r) => [r.name, String(r.actions)]);
  y += 0;
  renderSimpleTableSection(
  ...
  const trendTitle =
    summary.windowKind === "year"
      ? "Yearly Trend (12 months)"
      : summary.windowKind === "month"
      ? "Monthly Trend (4 weeks)"
      : summary.windowKind === "week"
      ? "Weekly Trend (7 days)"
      : summary.bucketMode === "yearly"
      ? summary.isAllTime
        ? "All-time Trend (by year)"
        : "Yearly Trend (by year)"
      : summary.bucketMode === "monthly"
      ? "Monthly Trend (last 12 months)"
      : "Daily Trend (last 31 days)";
  ...
  if (summary.bucketMode === "monthly")
    maxRows = Math.min(12, summary.buckets.length);
  else if (summary.bucketMode === "daily")
    maxRows = Math.min(31, summary.buckets.length);
  // For yearly we show all years; the table auto-breaks pages.
  const rows =
    summary.bucketMode === "yearly"
      ? summary.buckets
      : summary.buckets.slice(-maxRows);
  if (!rows.length) {
    drawEmptyBox("No data.", margin, y, pageW - margin * 2, 36);
    y += 44;
  } else {
    ...
    const tableRows: string[][] = rows.map((r) => [
      ...
      String(r.views),
      String(r.completions),
      r.views > 0 ? pct((r.completions / r.views) * 100) : "--%",
    ]);
    y =
      drawTable(
        ["Period", "Views", "Completions", "Rate"],
        tableRows,
        margin,
        y + 0,
        [200, 80, 90, 70],
  ...
  doc.setTextColor(colors.mutedText.r, colors.mutedText.g, colors.mutedText.b);
  const ts = new Date();
  const timeStr = `${String(ts.getDate()).padStart(2, "0")}/${String(
    ts.getMonth() + 1
  ...
  const fileLabel = summary.periodLabel
    .replaceAll(" ", "")
    .replaceAll("→", "-")
    .replaceAll(":", "");
