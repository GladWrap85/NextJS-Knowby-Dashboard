...
const buildSeries = (
  ...
  const totals = { views: views.length, comps: comps.length };
...
const buildOptions = (
  ...
  return {
    ...
    grid: { ...(base.grid ?? {}), padding: { ...base.grid?.padding, right: 6, left: 6 } },
    tooltip: {
      ...(base.tooltip ?? {}),
      shared: true,
    ...
    <div className="flex flex-wrap items-center gap-2">
      {[
        ...
        <button
          ...
          className={cn(pill(metric === key, tone), "flex items-center gap-1 cursor-pointer px-2 py-1")}
        >
          {key === "both" ? (
            <>
...
/* =============================================================================
   KNOWBY / EMPLOYEE LIST COMPONENTS
   ============================================================================= */
  ...
  const toneClasses =
    tone === "emerald"
      ? {
        ...
        active: "border-emerald-500/70 bg-emerald-50 shadow-[0_0_0_1px_rgba(16,185,129,0.2)] dark:bg-emerald-500/15",
        text: "text-emerald-700 dark:text-emerald-200",
        badge: "border-emerald-200 bg-emerald-100 text-emerald-700 dark:border-emerald-500/60 dark:bg-emerald-500/20 dark:text-emerald-100",
      ...
      : {
        ...
        active: "border-violet-500/60 bg-violet-50 shadow-[0_0_0_1px_rgba(139,92,246,0.2)] dark:bg-violet-500/15",
        text: "text-violet-700 dark:text-violet-200",
        badge: "border-slate-200 bg-slate-100 text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200",
          ...
          <tbody>
            {paged.map((row, idx) => (
              <tr
                ...
                className={cn(
                  "[&>td]:py-1.5 [&>td]:px-3",
                  idx % 2 === 0 ? "bg-white/80 dark:bg-slate-800/70" : "bg-slate-50/80 dark:bg-slate-900/50",
                  "hover:bg-slate-100/80 dark:hover:bg-slate-800/80 transition-colors"
                )}
              >
                <td className="whitespace-nowrap max-w-[28ch] truncate text-slate-700 dark:text-slate-200" title={row.knowbyName}>
                  {row.knowbyName}
                </td>
                <td className="whitespace-nowrap max-w-[28ch] truncate text-slate-600 dark:text-slate-300" title={row.employeeName}>
                  {row.employeeName}
                </td>
...
export default function AnalyticsExplorer({ selectedDateRange }: Props) {
  ...
  const activityRows = useMemo(() => {
    ...
    const include = (row: RawRow, type: SelectionEventRow["type"]) => {
      ...
      if (selKnowbys.length && (!row.knowby_name || !selKnowbys.includes(row.knowby_name))) return;
      if (selEmployees.length && (!row.member_name || !selEmployees.includes(row.member_name))) return;
  ...
  if (status === "loading") {
    return <Card className="rounded-3xl p-5 md:p-6 border-0 shadow-xl/2 bg-card min-h-[340px]" />;
  }
              ...
              {summary.map((item) => (
                ...
                >
                  <div className="flex flex-col leading-tight">
                    <p className="text-[11px] font-semibold uppercase tracking-wide opacity-70">{item.title}</p>
            ...
            <div>
              ...
              {hasUsageSelection ? (
                ...
                <div className="space-y-1 text-[11px] text-muted-foreground/80">
                  <p>
                    Select at least one {usageView === "knowbys" ? "knowby" : "employee"} to review activity events.
                  </p>
                </div>
              )}
            </div>
