...
export default function InsightsCard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [completionsData, viewsData] = await Promise.all([
        completionsPromise,
        viewsPromise,
      ]);
    ...
    if (isFilteredGraphMode && currentSelectedKnowbys.length > 0) {
      ...
      switch (selectedTimeFrame) {
        case "7d":
          ...
          intervals = eachDayOfInterval({ start: startDate, end: now }).map(
            (day) => ({
        ...
        case "3m":
          ...
          intervals = eachWeekOfInterval(
            ...
            label: format(weekStart, "'Wk' w"),
          }));
          break;
        case "1y":
          startDate = subYears(now, 1);
          intervals = eachMonthOfInterval({ start: startDate, end: now }).map(
            (monthStart) => ({
              start: monthStart,
      ...
      currentSelectedKnowbys.forEach((name) => {
        ...
        if (selectedStat === "completionRate")
          newKeys.push(`${name} Completion Rate`);
      ...
      const newBarData = intervals.map((interval) => {
        const intervalData: { [key: string]: any } = {
        ...
        currentSelectedKnowbys.forEach((knowbyName) => {
          ...
          const rate =
            viewsInInterval > 0
              ? Math.round((completionsInInterval / viewsInInterval) * 100)
              : 0;

          if (newKeys.includes(`${knowbyName} Views`))
            intervalData[`${knowbyName} Views`] = viewsInInterval;
          if (newKeys.includes(`${knowbyName} Completions`))
            intervalData[`${knowbyName} Completions`] = completionsInInterval;
          if (newKeys.includes(`${knowbyName} Completion Rate`))
            intervalData[`${knowbyName} Completion Rate`] = rate;
        });
        return intervalData;
      });
  ...
  const handleKnowbySelection = (knowbyName: string) => {
    if (isFilteredGraphMode) {
      ...
      setSelectedKnowbys(
        selectedKnowbys.includes(knowbyName) ? [] : [knowbyName]
      );
    }
  };
  ...
  const chartTitle = useMemo(() => {
    ...
    const statLabel =
      STAT_OPTIONS.find((s) => s.value === selectedStat)?.label || "";
    const timeFrameLabel =
      TIME_FRAME_OPTIONS.find((t) => t.value === selectedTimeFrame)?.label ||
      "";
  ...
  const getColor = (bar: any) => {
    ...
    if (selectedStat === "viewsAndCompletions") {
      if (bar.id.startsWith(knowby1Name)) {
        return bar.id.includes("Views")
          ? KNOWBY_COLORS.knowby1.base
          : KNOWBY_COLORS.knowby1.light;
      }
      if (knowby2Name && bar.id.startsWith(knowby2Name)) {
        return bar.id.includes("Views")
          ? KNOWBY_COLORS.knowby2.base
          : KNOWBY_COLORS.knowby2.light;
      }
    } else {
      if (bar.id.startsWith(knowby1Name)) return KNOWBY_COLORS.knowby1.base;
      if (knowby2Name && bar.id.startsWith(knowby2Name))
        return KNOWBY_COLORS.knowby2.base;
    }
    return "#ccc"; // Fallback
  };
          ...
          <div className="relative h-[130px] overflow-hidden">
            ...
            <div
              ...
              style={
                {
                  background:
                    "linear-gradient(to top, var(--card-background) 0%, transparent 100%)",
